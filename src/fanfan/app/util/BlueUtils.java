...
public class BlueUtils {
	...
	public void connect(String address) {
		...
		try {
			...
			// 判断是否是6.0版本以上
			if (Build.VERSION.SDK_INT > 22 && hasBLE) {
		...
		for (BluetoothGattService gattService : gattServices) {
			for (BluetoothGattCharacteristic characteristic : gattService.getCharacteristics()) {
				if (characteristic.getWriteType() == BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE) {
					Log.e("BlueUtils", "S-UUID:" + gattService.getUuid() + " S-Type" + gattService.getType()
							+ " writeType:" + characteristic.getWriteType() + " C-UUID:" + characteristic.getUuid());
	...
	public void bond() {
		try {
			...
			((Boolean) mCurDevice.getClass().getMethod("removeBond", new Class[0]).invoke(mCurDevice, new Object[0]))
					.booleanValue();
		} catch (Exception e) {
			e.printStackTrace();
		}
