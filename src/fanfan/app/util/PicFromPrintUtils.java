...
public class PicFromPrintUtils {
	 ...
	 * 360*360的图片，8个字节（8个像素点）是一个二进制，将二进制转化为十进制数值 y轴：24个像素点为一组，即360就是15组（0-14）
	 * x轴：360个像素点（0-359） 里面的每一组（24*360），每8个像素点为一个二进制，（每组有3个，3*8=24）
	 **************************************************************************/
	/**
	...
	public static byte[] draw2PxPoint(Bitmap bit) {
		int bitMapHeight = bit.getHeight();
		int printLine = (bitMapHeight / 24); // (bitMapHeight % 24 == 0 ? 0 : 1) +
		byte[] data = new byte[(3 * bitMapHeight + 6) * printLine]; // new byte[16290];
		int k = 0;
		for (int j = 0; j < printLine; j++) {
			data[k++] = 0x1B;
			data[k++] = 0x2A;
			...
			for (int x = 0; x < 360; x++) {
				for (int m = 0; m < 3; m++) {
					for (int n = 0; n < 8; n++) {
						byte b = px2Byte(x, j * 24 + m * 8 + n, bit);
						data[k] += data[k] + b;
					}
					k++;
				}
